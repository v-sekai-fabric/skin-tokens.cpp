    ...
    for (std::size_t joint = 0; joint < output.normalized_joints.size(); ++joint) {
        ...
        if (branch) {
            if (parent < 0 || static_cast<std::size_t>(parent) >= joint)
                return std::unexpected(fail(error_code::invalid_argument, "skeleton parent order is invalid"));
...
result<skeleton> detokenize_generated_skeleton(
    std::span<const std::int32_t> tokens, vec3 center, float scale) try {
    if (tokens.size() < 6U || tokens.front() != 257 || tokens.back() != 258 ||
        !std::isfinite(scale) || scale <= 0.0F)
        return std::unexpected(fail(error_code::invalid_argument, "invalid generated skeleton token sequence"));
    ...
    if (output.names.empty() || output.names.size() > 256U)
        return std::unexpected(fail(error_code::compute, "TokenRig generated an empty or oversized skeleton"));
    return output;
