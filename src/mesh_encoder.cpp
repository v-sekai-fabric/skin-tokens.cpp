...
result<tensor_snapshot> run_mesh_encoder_fixture(const weight_component & weights, ggml_backend_t backend,
                                                  ...
                                                  std::span<const float> normals) try {
    if (backend == nullptr || points.size() != static_cast<std::size_t>(points_count * 3) || normals.size() != points.size())
        return std::unexpected(fail(error_code::invalid_argument, "mesh fixture must contain 64 point/normal triples"));
    ...
    if (context == nullptr) return std::unexpected(fail(error_code::allocation, "cannot create mesh encoder graph"));
    ...
    if (allocator == nullptr || !ggml_gallocr_reserve(allocator, graph) || !ggml_gallocr_alloc_graph(allocator, graph)) {
        ...
        return std::unexpected(fail(error_code::allocation, "cannot allocate mesh encoder graph"));
    }
    ...
    if (ggml_backend_graph_compute(backend, graph) != GGML_STATUS_SUCCESS)
        return std::unexpected(fail(error_code::compute, "mesh encoder graph execution failed"));
...
result<std::vector<float>> encode_mesh(const weight_component & weights, ggml_backend_t backend,
                                       ...
                                       std::span<const std::int32_t> query_indices) try {
    if (backend == nullptr || points.empty() || normals.size() != points.size() || query_indices.empty() ||
        query_indices.size() > points.size())
        return std::unexpected(fail(error_code::invalid_argument, "invalid mesh encoder input"));
    for (const auto index : query_indices)
        if (index < 0 || static_cast<std::size_t>(index) >= points.size())
            return std::unexpected(fail(error_code::invalid_argument, "mesh encoder query index is out of range"));
