...
int main(int argc, char ** argv) try {
    ...
    for (std::size_t beam = 0; beam < 2U; ++beam) {
        ...
        for (std::size_t index = 0; index < vocabulary; ++index) {
            const float difference = (*actual)[beam * vocabulary + index] - expected[index];
        ...
        const auto actual_top = std::max_element(actual->begin() + static_cast<std::ptrdiff_t>(beam * vocabulary + 267U),
            actual->begin() + static_cast<std::ptrdiff_t>(beam * vocabulary + 33035U));
        const auto expected_top = std::max_element(expected.begin() + 267, expected.begin() + 33035);
        const auto actual_id = static_cast<std::size_t>(actual_top - actual->begin()) - beam * vocabulary;
        const auto expected_id = static_cast<std::size_t>(expected_top - expected.begin());
        std::cout << "beam=" << beam << " max_abs=" << maximum << " rel_l2=" << relative
                  << " top=" << actual_id << " serial_top=" << expected_id << '\n';
        passed = passed && relative < 1e-5 && maximum < 1e-4F && actual_id == expected_id;
    }
    return passed ? 0 : 1;
